...
public class ExportProjectActivity extends BaseAppCompatActivity {
	...
	private Button export_source_open_folder;
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		export_source_open_folder = findViewById(R.id.export_source_open_folder_button);
		...
		Objects.requireNonNull(getSupportActionBar()).setTitle(Helper.getResString(R.string.myprojects_export_project_actionbar_title));
	...
	private void initializeExportSrcViews() {
		...
		export_source_open_folder.setOnClickListener(v -> openExportedSourceCode());
	...
	private void openExportedSourceCode() {
		if (! export_src_filename.isEmpty()) {
			File exportedFile = new File(export_src_full_path, export_src_filename);
			// Same as BuildConfig.APPLICATION_ID + ".provider"
			Uri uri = FileProvider.getUriForFile(
					getApplicationContext(),
					getPackageName() + ".provider",
					exportedFile);
			Intent viewIntent = new Intent(Intent.ACTION_VIEW);
			viewIntent.setDataAndType(uri, "application/zip");
			viewIntent.addFlags(Intent.FLAG_GRANT_READ_URI_PERMISSION);
			startActivity(Intent.createChooser(viewIntent, "My Projects - Export source code"));
		}
	}
