...
public class ResourceCompiler {
	...
	static class Aapt2Compiler implements Compiler {
		...
		private void compileLibrariesInParallel(String outputPath) throws zy, MissingFileException {
			...
			ExecutorService executor = Executors.newFixedThreadPool(Math.max(totalTasks, Runtime.getRuntime().availableProcessors()));
		...
		private void link() throws zy, MissingFileException {
			...
			// Criar coleções locais para assets paralelos
			List<String> assetsPaths = Stream.concat(
							...
							Stream.of(buildHelper.fpu.getPathAssets(buildHelper.yq.sc_id))
					                           ...
					                           .filter(FileUtil::isExistFile)
					                           .distinct()
					                           .toList();

			List<String> libraryAssets = buildHelper.builtInLibraryManager.getLibraries().parallelStream()
					                             .filter(Jp::hasAssets)
					                             .map(library -> BuiltInLibraries.getLibraryAssetsPath(library.getName()))
					                             .peek(file -> {
						                             try {
							                             linkingAssertDirectoryExists(file);
			...
			// Unir todas as listas locais em args de forma thread-safe
			synchronized (args) {
				// Combinar e adicionar os assets
				Stream.concat(
						Stream.concat(assetsPaths.stream(), libraryAssets.stream()),
			...
			// Adicionar recursos compilados das bibliotecas built-in
			...
			// Incluir recursos locais compilados em paralelo
			File[] filesInCompiledResourcesPath = new File(resourcesPath).listFiles();
			if (filesInCompiledResourcesPath != null) {
				Arrays.stream(filesInCompiledResourcesPath)
						.parallel()
						.filter(file -> file.isFile() &&
								                ! file.getName().equals("project.zip") &&
			...
			// Incluir recursos do projeto e importados
